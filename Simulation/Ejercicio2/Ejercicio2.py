...
class Aeropuerto:
    ...
    def __init__(self):
        ...
        self.n1 = self.n2 = self.n3 = self.n14 = self.total_aviones = 0
        # Almacena el id de los aviones que estan esperando en este instante en el nodo correspondiente
        # solo el de los que estan esperando, no el de los que estan ya recibiendo servicio
        self.aviones_n1 = []
        self.aviones_n2 = []
        self.aviones_n3 = []
        self.aviones_n14 = []
        ...
        self.t_m_e_ll = self.t_m_e_ss = self.tiempo_pistas_ocupadas = 0
         # Número medio de aviones en cada nodo hasta el instante j-esimo
        self.n_m1 = self.n_m2 = self.n_m3 = self.n_m14 = 0
        # t: Tiempo actual en el que nos encontramos
        self.t = 0
        # Número de llegadas totales a cada nodo hasta el instante t (acumulado)
        self.nll1 =  self.nll2 = self.nll3 = self.nll14 = 0
         # Número de salidas totales a cada nodo hasta el instante t (acumulado)
        self.ns1 =  self.ns2 = self.ns3 = self.ns14 = 0
        # Instante de tiempo en el que deja de esperar cada avión en cada nodo
        self.espera_n1 = {}
        self.espera_n2 = {}
        self.espera_n3 = {}
        self.espera_n14 = {}
    ...
    def getTDesembarque(self):
        ...
        return np.random.exponential(48.89)

    def getTDespegue(self):
        return np.random.uniform(10,15)

    def auxAvion(self, instante_tiempo):
        instante_tiempo = (instante_tiempo / MINUTOS_HORA) % HORAS_DIA
    ...
    def getTExponential(self):
        #TODO
        return np.random.exponential(1/(TASA_EXPONENCIAL/60))
    ...
    def servicio_nodo1(self, event):
        ...
        if self.n_esperando_1 > 0:
            av_uid = self.aviones_n1[0]
            self.aviones_n1.remove(av_uid)
            self.espera_n1[av_uid] = tsuc
            self.n_esperando_1 -= 1
            self.pistas_ocupadas += 1
            t_aterrizaje = self.getTAterrizaje()
            ...
            av_uid = self.aviones_n14[0]
            self.aviones_n14.remove(av_uid)
            self.espera_n14[av_uid] = tsuc
    ...
    def servicio_nodo2(self, event):
        ...
        if self.n2 >= VEHICULOS:
            t_vehiculo = self.getTExponential()
            av_uid = self.aviones_n2[0]
            self.espera_n2[av_uid] = tsuc
            self.aviones_n2.remove(av_uid)
    ...
    def servicio_nodo3(self, event):
        ...
        if self.n3 >= PUERTAS:
            t_desembarque = self.getTDesembarque()
            av_uid = self.aviones_n3[0]
            self.aviones_n3.remove(av_uid)
            self.espera_n3[av_uid] = tsuc
            self.setEventTime('sn3', self.t + t_desembarque, av_uid)

        # Ponemos comprobamos si hay pistas libres
        if self.pistas_ocupadas < PISTAS:
            self.aviones_n14.remove(uid)
            self.espera_n14[uid] = tsuc
            self.n_esperando_14 -= 1
            self.pistas_ocupadas += 1
            t_despegue = self.getTDespegue()
            self.setEventTime('sn14', self.t + t_despegue, uid)
...
max_llegada = 0
acumulado1 = acumulado2 = acumulado3 = acumulado14 = t_espera_n1 = t_espera_n14 = 0
for indi in a.ll1:
    tmp = a.espera_n1[indi] - a.ll1[indi]
    if tmp > max_llegada: max_llegada = tmp
    t_espera_n1 += tmp

acumulado1 = sum(a.ss1[indi] - a.ll1[indi] for indi in a.ll1)
acumulado2 = sum(a.ss2[indi] - a.ll2[indi] for indi in a.ll2)
acumulado3 = sum(a.ss3[indi] - a.ll3[indi] for indi in a.ll3)
acumulado14 = sum(a.ss14[indi] - a.ll14[indi] for indi in a.ll14)

max_salida = 0
for indi in a.ll14:
    tmp = a.espera_n14[indi] - a.ll14[indi]
...
media_aviones_sistema = a.total_aviones/T

pp("Total aviones: {}".format(a.total_aviones))
...
pp("Tiempo máximo entre espera y servicio para despegar: {}".format(max_salida))
pp("Tiempo medio de espera llegadas: {}".format(t_espera_n1/a.total_aviones))
pp("Tiempo medio de espera salidas: {}".format(t_espera_n14/a.total_aviones))
pp("Tiempo medio de las pistas ocupadas (solo cuando las {} pistas estan siendo utilizadas a la vez): {}%".format(PISTAS, round((a.tiempo_pistas_ocupadas/T)*100, 2)))
